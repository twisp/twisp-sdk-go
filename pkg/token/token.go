...
import (
	...
	///	"github.com/txlayer/txlayer/lib/auth/arn"
)
...
const (
	...
	v1Prefix               = "twisp-aws-v1."
	maxTokenLenBytes       = 1024 * 4
	clusterIDHeader        = "x-twisp-aws-id"
...
func stsHostsForPartition(partitionID string) map[string]bool {
	...
	for _, ep := range stsSvc.Endpoints() {
...
func Canonicalize(a string) (string, error) {
	parsed, err := arn.Parse(a)
	if err != nil {
		return "", fmt.Errorf("arn '%s' is invalid: '%v'", a, err)
	}

	if err := checkPartition(parsed.Partition); err != nil {
		return "", fmt.Errorf("arn '%s' does not have a recognized partition", a)
	}
	...
	case "sts":
		...
		case "assumed-role":
			if len(parts) < 3 {
				return "", fmt.Errorf("assumed-role arn '%s' does not have a role", a)
			}
	...
	return "", fmt.Errorf("service %s in arn %s is not a valid service for identities", parsed.Service, a)
}
